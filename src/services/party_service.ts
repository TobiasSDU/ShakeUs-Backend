...
export class PartyService {
  ...
  public static async updateActivityPack(
    ...
    if (await this.isUserPrimaryHost(primaryHostId, partyId)) {
      ...
      if (updateResult.modifiedCount == 1) {
        const newActivityPack = await ActivityPackService.showActivityPack(
          activityPackId
        );
          ...
          if (newHost) {
            socketService.emitToRoom(
              "guest-promoted",
              {
                ...
                message: newHost.getName + " has been promoted to host",
              },
              partyId
            );
          }
  ...
  public static async removeGuest(
    ...
    if (await this.isUserAHost(hostId, partyId)) {
      ...
      if (removeResult.modifiedCount == 1) {
        const deleteResult = await GuestService.deleteGuest(removedGuestId);
  ...
  public static async leaveParty(partyId: string, userId: string) {
    ...
    if (!isPrimaryHost && party) {
      const userObject: Guest | undefined = await GuestService.getGuestInfo(
        userId
      );
  ...
  public static async deleteParty(partyId: string, primaryHostId: string) {
    const collection = await this.getPartiesCollection();
    const isPrimaryHost = await this.isUserPrimaryHost(primaryHostId, partyId);

    if (isPrimaryHost) {
      const party = await this.getPartyInfo(partyId, primaryHostId);

      if (party) {
        const deleteUsersResult = await this.deleteAllGuestsAndHosts(partyId);

        const deleteActivitiesResult =
          await ActivityPackService.deleteActivityPack(party.getActivityPackId);
