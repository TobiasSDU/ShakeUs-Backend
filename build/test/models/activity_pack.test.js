...
const originalActivityPack = new activity_pack_1.ActivityPack(testId, testTitle, testDescription, []);
describe('activity pack model methods return expected values', () => {
    let testPack;
    beforeEach(() => {
        testPack = Object.assign(Object.getPrototypeOf(originalActivityPack), originalActivityPack);
    });
    ...
    it('is possible to remove an activity from the activities array', () => {
        ...
        expect(testPack.getActivities).toEqual(expect.not.arrayContaining(['activity3']));
    });
